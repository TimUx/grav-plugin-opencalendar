# OpenCalendar documentation

Documentation is available in English and German.

## English / Englisch

→ **[docs/en/](en/)**

| Document | Topic |
|----------|-------|
| [Installation](en/Installation.md) | Setup and requirements |
| [Configuration](en/Configuration.md) | All config options |
| [Architecture](en/Architecture.md) | Code structure |
| [Sources](en/Sources.md) | ICS, CalDAV, JSON, local, Admin upload |
| [ICS](en/ICS.md) | iCalendar specifics |
| [Synchronization](en/Synchronization.md) | Sync, cleanup, Admin upload |
| [SQLite](en/SQLite.md) | Database storage |
| [Twig](en/Twig.md) | Template integration |
| [Shortcodes](en/Shortcodes.md) | Page shortcodes |
| [Subscribe](en/Subscribe.md) | Network calendar / phone subscription |
| [Searching](en/Searching.md) | Full-text search |
| [Filtering](en/Filtering.md) | Event filters |
| [Caching](en/Caching.md) | Cache layers |
| [API](en/API.md) | REST endpoints |
| [Development](en/Development.md) | Contributor guide |
| [Migration](en/Migration.md) | Upgrades |
| [Troubleshooting](en/Troubleshooting.md) | Problem solving |
| [FAQ](en/FAQ.md) | Common questions |
| [Publishing / GPM](en/Publishing-GPM.md) | Official Grav repository listing |

## Deutsch

→ **[docs/de/](de/)**

| Dokument | Thema |
|----------|-------|
| [Installation](de/Installation.md) | Einrichtung und Voraussetzungen |
| [Konfiguration](de/Configuration.md) | Alle Konfigurationsoptionen |
| [Architektur](de/Architecture.md) | Code-Struktur |
| [Quellen](de/Sources.md) | ICS, CalDAV, JSON, lokal, Admin-Upload |
| [ICS](de/ICS.md) | iCalendar-Details |
| [Synchronisation](de/Synchronization.md) | Sync, Cleanup, Admin-Upload |
| [SQLite](de/SQLite.md) | Datenbankspeicher |
| [Twig](de/Twig.md) | Template-Integration |
| [Shortcodes](de/Shortcodes.md) | Seiten-Shortcodes |
| [Abonnieren](de/Subscribe.md) | Kalender auf dem Smartphone abonnieren |
| [Suche](de/Searching.md) | Volltextsuche |
| [Filter](de/Filtering.md) | Terminfilter |
| [Caching](de/Caching.md) | Cache-Schichten |
| [API](de/API.md) | REST-Endpunkte |
| [Entwicklung](de/Development.md) | Leitfaden für Mitwirkende |
| [Migration](de/Migration.md) | Upgrades |
| [Fehlerbehebung](de/Troubleshooting.md) | Problemlösung |
| [FAQ](de/FAQ.md) | Häufige Fragen |
| [Veröffentlichung / GPM](de/Publishing-GPM.md) | Offizielle Grav-Repository-Aufnahme |

## Project root / Projektstamm

| English | Deutsch |
|---------|---------|
| [README.md](../README.md) | [README.de.md](../README.de.md) |
| [CONTRIBUTING.md](../CONTRIBUTING.md) | [CONTRIBUTING.de.md](../CONTRIBUTING.de.md) |
| [SECURITY.md](../SECURITY.md) | [SECURITY.de.md](../SECURITY.de.md) |
| [CODE_OF_CONDUCT.md](../CODE_OF_CONDUCT.md) | [CODE_OF_CONDUCT.de.md](../CODE_OF_CONDUCT.de.md) |
| [CHANGELOG.md](../CHANGELOG.md) | — (GPM format, English) |
